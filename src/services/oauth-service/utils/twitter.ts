...
export function calculateReputation(data: TwitterProfile): number {
  ...
  const tweetCountWeight = 0.2;
  const listedCountWeight = 0.1;
  const likeCountWeight = 0.1;
  const verifiedWeight = 0.2;
  const descriptionWeight = 0.1;
  const createdAtWeight = 0.1; // Weight for account creation date

  // Extract data from twitter profile
  const { followersCount, tweetCount, listedCount, likeCount, verified, description, createdAt } = data;
