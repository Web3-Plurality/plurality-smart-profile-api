  ...
  new OAuthSnapChatStrategy(
    {
      ...
      scope:
        'https://auth.snapchat.com/oauth2/api/user.display_name https://auth.snapchat.com/oauth2/api/user.bitmoji.avatar https://auth.snapchat.com/oauth2/api/user.external_id', //space
      state: true,
      pkce: true,
    },
    // Verify callback
...
snapchatRouter.get(
  '/callback',
  passport.authenticate('snapchat', { session: false }),
...
snapchatRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      Logger.info(
        `${SNAPCHAT_APP}: Request for information has been received successfully with id ${req.accessTokenID}`,
      );
