...
import { createPrompt, FACEBOOK_FETCH_INTEREST_FROM_NAMES_PROMPT, FACEBOOK_FETCH_INTEREST_PROMPT } from "../utils/aiPrompts";
...
facebookRouter.get('/info', hasValidAccessTokenHeader, isAuthenticated, isProfileMapEmpty, async (req, res) => {
    try {
        ...
        if (accessToken) {
            ...
            userProfile.scores.push({ score_type: SCORE_TYPES.REPUTATION_SCORE, score_value: facebookProfile?.reputationScore });
